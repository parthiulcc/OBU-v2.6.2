...
/**
 * Upgrade library code for the vmatch question type.
 *
 * @package    qtype
 * @subpackage vmatch
...
/**
 * Class for converting attempt data for vmatch questions when upgrading
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_vmatch_qe2_attempt_updater extends question_qtype_attempt_updater {
    protected $stems;
    protected $choices;
    protected $right;
    protected $stemorder;
    ...
    public function question_summary() {
        ...
        foreach ($this->question->options->subquestions as $vmatchsub) {
            $ans = $vmatchsub->answertext;
            $key = array_search($vmatchsub->answertext, $this->choices);
            if ($key === false) {
                $key = $vmatchsub->id;
                $this->choices[$key] = $vmatchsub->answertext;
            }

            if ($vmatchsub->questiontext !== '') {
                $this->stems[$vmatchsub->id] = $this->to_text($vmatchsub->questiontext);
                $this->right[$vmatchsub->id] = $key;
            }
        }
    ...
    protected function lookup_choice($choice) {
        foreach ($this->question->options->subquestions as $vmatchsub) {
            if ($vmatchsub->code == $choice) {
                if (array_key_exists($vmatchsub->id, $this->choices)) {
                    return $vmatchsub->id;
                } else {
                    return array_search($vmatchsub->answertext, $this->choices);
                }
            }
        }
        return null;
            ...
            if (array_key_exists($stemid, $this->stems) && $choices[$stemid]) {
                ...
                if ($choiceid) {
                    ...
                    $this->logger->log_assumption("Dealing with a place where the
                            ...
                            vmatch question {$this->question->id}");
    ...
    public function supply_missing_first_step_data(&$data) {
        throw new coding_exception('qtype_vmatch_updater::supply_missing_first_step_data ' .
                'not tested');
