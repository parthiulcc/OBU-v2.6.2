...
/**
 * Defines the editing form for the vmatch question type.
 *
 * @package    qtype
 * @subpackage vmatch
...
/**
 * vmatch question type editing form definition.
 *
 * @copyright  2007 Jamie Pratt me@jamiep.org
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_vmatch_edit_form extends question_edit_form {
    ...
    protected function definition_inner($mform) {
        $mform->addElement('advcheckbox', 'shuffleanswers',
                get_string('shuffle', 'qtype_vmatch'), null, null, array(0, 1));
        $mform->addHelpButton('shuffleanswers', 'shuffle', 'qtype_vmatch');
        $mform->setDefault('shuffleanswers', 1);

        $mform->addElement('static', 'answersinstruct',
                get_string('availablechoices', 'qtype_vmatch'),
                get_string('filloutthreeqsandtwoas', 'qtype_vmatch'));
    ...
    public function validation($data, $files) {
        ...
        foreach ($questions as $key => $question) {
            ...
            if ($trimmedquestion != '' && $trimmedanswer == '') {
                $errors['subanswers['.$key.']'] =
                        get_string('novmatchinganswerforq', 'qtype_vmatch', $trimmedquestion);
            }
        }
        ...
        if ($questioncount < 1) {
            $errors['subquestions[0]'] =
                    get_string('notenoughqsandas', 'qtype_vmatch', $numberqanda);
        }
        if ($questioncount < 2) {
