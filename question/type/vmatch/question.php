...
/**
 * vmatching question definition class.
 *
 * @package    qtype
 * @subpackage vmatch
...
/**
 * Represents a vmatching question.
 *
 * @copyright  2009 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_vmatch_question extends question_graded_automatically_with_countback {
    ...
    public $stems;
    /** @var array of choices that can be vmatched to each stem. */
    public $choices;
    ...
    public function summarise_response(array $response) {
        ...
        foreach ($this->stemorder as $key => $stemid) {
            if (array_key_exists($this->field($key), $response) && $response[$this->field($key)]) {
                $vmatches[] = $this->html_to_text($this->stems[$stemid],
    ...
    public function get_validation_error(array $response) {
        ...
        return get_string('pleaseananswerallparts', 'qtype_vmatch');
    }
    ...
    public function check_file_access($qa, $options, $component, $filearea, $args, $forcedownload) {
        if ($component == 'qtype_vmatch' && $filearea == 'subquestion') {
