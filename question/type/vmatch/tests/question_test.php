...
/**
 * Unit tests for the vmatching question definition classes.
 *
 * @package    qtype
 * @subpackage vmatch
...
/**
 * Unit tests for the vmatching question definition class.
 *
 * @copyright  2009 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_vmatch_question_test extends advanced_testcase {

    public function test_get_expected_data() {
        $question = test_question_maker::make_a_vmatching_question();
    ...
    public function test_get_question_summary() {
        $vmatch = test_question_maker::make_a_vmatching_question();
        $vmatch->start_attempt(new question_attempt_step(), 1);
        $qsummary = $vmatch->get_question_summary();
        $this->assertRegExp('/' . preg_quote($vmatch->questiontext) . '/', $qsummary);
        foreach ($vmatch->stems as $stem) {
            $this->assertRegExp('/' . preg_quote($stem) . '/', $qsummary);
        }
        foreach ($vmatch->choices as $choice) {
    ...
    public function test_summarise_response() {
        $vmatch = test_question_maker::make_a_vmatching_question();
        $vmatch->shufflestems = false;
        $vmatch->start_attempt(new question_attempt_step(), 1);

        $summary = $vmatch->summarise_response(array('sub0' => 2, 'sub1' => 1));
    ...
    public function test_classify_response() {
        ...
        $choiceorder = $vmatch->get_choice_order();
        $orderforchoice = array_combine(array_values($choiceorder), array_keys($choiceorder));
        $choices = array(0 => get_string('choose') . '...');
        foreach ($choiceorder as $key => $choice) {
            $choices[$key] = $vmatch->choices[$choice];
        }
        ...
        $this->assertEquals(array(
                ...
                ), $vmatch->classify_response(array('sub0' => $orderforchoice[2],
        ...
        $this->assertEquals(array(
                ...
                ), $vmatch->classify_response(array('sub0' => $orderforchoice[1],
                        'sub1' => $orderforchoice[2], 'sub2' => $orderforchoice[2],
                        'sub3' => $orderforchoice[1])));
    }
}
