...
/**
 * vmatching question renderer class.
 *
 * @package    qtype
 * @subpackage vmatch
...
/**
 * Generates the output for vmatching questions.
 *
 * @copyright  2009 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_vmatch_renderer extends qtype_with_combined_feedback_renderer {
    ...
    public function formulation_and_controls(question_attempt $qa,
        ...
        foreach ($stemorder as $key => $stemid) {
            ...
            $result .= html_writer::tag('td', $question->format_text(
                    ...
                    $qa, 'qtype_vmatch', 'subquestion', $stemid),
                    array('class' => 'text'));
    ...
    public function correct_response(question_attempt $qa) {
        ...
        foreach ($stemorder as $key => $stemid) {
            $right[] = $question->format_text($question->stems[$stemid],
                    ...
                    'qtype_vmatch', 'subquestion', $stemid) . ' – ' .
                    $choices[$question->get_right_choice_for($stemid)];
        }

        if (!empty($right)) {
            return get_string('correctansweris', 'qtype_vmatch', implode(', ', $right));
        }
    }
}
