...
$redirectionUrl='welcome.php?id=' . $params["enc_course_id"] . '&' . 
                voiceboard_WIMBA_get_url_params($params["enc_course_id"]) . '&time=' . $session->timeOfLoad;
if ( $session->error === false && $session != NULL ) 
{
  ...
  $urlParams = 'resource_id='.$resource_id.'&type=board&id=' . $params["enc_course_id"] . '&' . 
                voiceboard_WIMBA_get_url_params($params["enc_course_id"]) . '&time=' . $session->timeOfLoad;
        ...
        <div class="headerBar">
         ...
         <div class="contextBar" style="padding-top: 10px; padding-left:10px; height:35px">
            <div style="display: block; float: left;">
                <span style="float: left; width: 130px; font-size: 11px; text-align: right; padding-right: 5px;font-weight: bold;"><?php echo get_string('grade_vb_name', 'voiceboard');?></span><span><?php echo $resource->WIMBA_getTitle();?></span> <br/> <span style="float: left; width: 130px; font-size: 11px; text-align: right; padding-right: 5px; font-weight: bold;"><?php echo get_string('points_possible', 'voiceboard');?></span><span>  <?php echo $options->WIMBA_getPointsPossible();?></span>
            </div>
             <div style="float: right;padding-right:15px">
               <input type="button" onclick="javascript:WIMBA_LaunchVoiceTools('manageAction.php','<?php echo $urlParams;?>');" value="<?php echo get_string('grade_open_board', 'voiceboard');?>"/>
            </div>
        </div>
         <div style="ooverflow: hidden; width: 700px; background-color: rgb(228, 233, 237); height: 30px;border-top:1px solid #718a9d;border-bottom:1px solid #718a9d;">
           <table width="685px"  cellspacing="0" cellpadding="1" border="0" style="float: left;">
        	<tr style="background-color:#E4E9ED;height:30px;" >
        		<th><span style="display:block;width:140px;text-align:left;display:block;font-size: 11px;"><?php echo get_string('grade_last_name', 'voiceboard');?></span></th>
        		<th><span style="display:block;width:140px;text-align:left;display:block;font-size: 11px;"><?php echo get_string('grade_first_name', 'voiceboard');?></span></th>
        		<th><span style="display:block;width:140px;text-align:left;display:block;font-size: 11px;"><?php echo get_string('grade_user_name', 'voiceboard');?></span></th>
        		<th><span style="display:block;width:65px;font-size: 11px;"><?php echo get_string('grade_posts', 'voiceboard');?></span></th>
        		<th><span style="display:block;width:100px;font-size: 11px;"><?php if($isAverageMethodAvailable){ echo get_string('grade_avg_length', 'voiceboard'); }?></span></th>
        		<th><span style="display:block;width:100px;font-size: 11px;"><?php echo get_string('grade_points', 'voiceboard');?></span></th>
        	</tr>
           </table>
          </div>
        	...
        	<ul class="regular_btn_list" style="float: right; padding-top: 2px;">
	        	<li>
	        		<a class="regular_btn" href="#" onclick="javascript:top.location='<?php echo $cancelUrl?>';">
	        			<span style="width: 110px;"><?php echo get_string('validationElement_cancel','voiceboard');?></span>
	        		</a>
	        	</li>
		        <li>
		        	<input type="submit" class="regular_btn-submit" onclick="javascript:WIMBA_submitGradeForm('manageAction.php','submitGrade','<?php echo $urlParams;?>')" value="<?php echo get_string('validationElement_saveAll','voiceboard');?>"/>
		        </li>
        	</ul>
        </div>
