...
$urlModuleForm=$CFG->wwwroot.'/course/mod.php?section=0&sesskey='.sesskey().
                '&id='.$session->WIMBA_getCourseId().'&add=voiceboard&rid=';
...
if ( $session->error === false && $session != NULL ) 
	...
	if ($params['action'] == 'launch') 
	{
		...
		if ($result != NULL) 
		{
		    wimba_add_log(WIMBA_DEBUG,voiceboard_LOGS,"launch the ". $params["type"] . ", nid =" .$result->WIMBA_getNid()); 
	...
	elseif ($params['action'] == 'create' || $params['action'] == "createDefault")  
	{
		...
		if ($result != NULL && $result->error != "error") 
		{
			...
			$resource_id = WIMBA_storeResource($result->WIMBA_getRid(), $session->WIMBA_getCourseId(), $params, "voiceboard");
                                                              			
			if (empty ($resource_id)) 
			{
				wimba_add_log(WIMBA_ERROR,voiceboard_LOGS,"manageAction : Problem to add the resource into the database"); 
		...
		else 
		{
		   if($params['action'] == "createDefault")
            ...
            wimba_add_log(WIMBA_ERROR,voiceboard_LOGS,"manageAction :Problem to add the new resource on the vt server"); 
			WIMBA_redirection($redirectionUrl . '&error=problem_vt');
		}
	}
	elseif ($params['action'] == 'update') 
	{
		if ($params['type'] == "board") 
		{
			...
			if ($result != NULL && $result->error != "error") 
    		{
    			if($params['grade'] == 'true'){         
    			   $params['gradeid']=voiceboard_add_grade_column($result->WIMBA_getRid(),$params["enc_course_id"],$result->WIMBA_getTitle(),$params["points_possible"]);
    			}else{
    			  $resourceDb=$DB->get_record("voiceboard_resources",array("rid" => $result->WIMBA_getRid()));
    			  if($resourceDb->gradeid != -1 ){
        			   voiceboard_delete_grade_column($result->WIMBA_getRid(),$params["enc_course_id"]);
		...
		if ($result != NULL) 
		{
			...
			$resource_id = WIMBA_updateResource($result->WIMBA_getRid(), $session->WIMBA_getCourseId(), $params, "voiceboard");
	...
	elseif ($params['action'] == 'delete') 
	{
	    ...
	    $resourceDb=$DB->get_record("voiceboard_resources", array("rid" => $params["resource_id"]));
            if($resourceDb->gradeid != -1 ){
	      voiceboard_delete_grade_column($params["resource_id"],$params["enc_course_id"]);
	    }
		$result = $vtAction->WIMBA_deleteResource($params["resource_id"]);
		if ($result != NULL) { //if no error during the creation
			if (! voiceboard_delete_all_instance_of_resource($params["resource_id"])) 
			{
	...
	{
		voiceboard_add_grades($params["resource_id"],$params["enc_course_id"], $params["grades"]);
