...
function create(name,courseid){
    if (name == '')
        return false;
    createNewResource(M.cfg.wwwroot+"/mod/voiceboard/manageAction.php","voicetools","board",name,$F($('mform1')["url_params"]));
}

function LoadNewFeaturePopup(current)
{
    if( current == "new" ){
        var ret = prompt('Please enter a title for the new Voice Board');
        var pattern=/^\s+$/g;
        if (pattern.test(ret)) {
          $('id_resource').selectedIndex=0;
          alert('Voice Board name can not be blank');
          return false;
        }
        if (ret == null) {
          $('id_resource').selectedIndex=0;
