...
if($uiManager->WIMBA_getSessionError() === false)
{
    /*******************
    ...
    { //get the information of the resource
        ...
        {
            wimba_add_log(WIMBA_ERROR,voiceboard_LOGS,"getXmlNewPanel : ". get_string('problem_vt','voiceboard')); 
            $uiManager->WIMBA_setError(get_string('problem_vt','voiceboard'));
        }
        else
        {
            $currentBoardInformations=voiceboard_get_wimbaVoice_Informations($params["resource_id"]);
            $uiManager->WIMBA_setCurrentProduct($typeProduct,$currentBoard,$currentBoardInformations);
        }
    }
    else
    {
        $uiManager->WIMBA_setCurrentProduct($typeProduct);
    }
    
    $display= $uiManager->WIMBA_getVTSettingsView($action,$createWorkflow);

}
else
{
    wimba_add_log(WIMBA_ERROR,voiceboard_LOGS,"getXmlNewPanel : ". get_string ('error_session', 'voiceboard')); 
