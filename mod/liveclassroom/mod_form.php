...
$PAGE->requires->js('/mod/liveclassroom/js/init_calendar_checkbox.js');
...
class mod_liveclassroom_mod_form extends moodleform_mod {
    ...
    function definition() {
        ...
        $api=new WIMBA_LCAction(null,$CFG->liveclassroom_servername, $CFG->liveclassroom_adminusername,
                $CFG->liveclassroom_adminpassword, $CFG->dataroot, $COURSE->id);

	if (!$api->api->WIMBA_isConfigured()) {
		print_error(get_string('error_unconfigured_lc','liveclassroom'));
	}
        ...
        $rooms = $api->WIMBA_getRooms($COURSE->id."_T");
        if ($rooms === false)
            print_error(get_string('error_connection_lc','liveclassroom'));

        $url_params = liveclassroom_WIMBA_get_url_params($course_id);
        ...
        foreach ($rooms as $room)
        {
            if ($room->WIMBA_isArchive() == false) {
                $resopts[$api->WIMBA_getPrefix().$room->WIMBA_getRoomId()] = (strlen($room->WIMBA_getLongname()) > 25) 
                                                            ? substr($room->WIMBA_getLongname(), 0, 25)."..." 
                                                            : $room->WIMBA_getLongname();
            }
        }
        $resopts['new'] = 'New room....';
        $resselc = isset($this->current->type) ? $this->current->type : 'empty';
        $mform->addElement('select', 'resource', get_string('liveclassroomtype', 'liveclassroom'), $resopts, array('onchange' => 'LoadNewFeaturePopup(this.options[this.selectedIndex].value);'));
        $mform->addRule('resource', null, 'required', null, 'client');
        $mform->setDefault('resource', $resselc);


        //calendar
        $mform->addElement('checkbox', 'calendar_event', get_string('add_calendar', 'liveclassroom'), '', array('onclick' => 'hideCalendarEvent("check");', 'id' => 'id_calendar_event', 'onblur' => 'hideCalendarEvent("check");'));
...
// buttons
    ...
    function validation($data, $files) {
       ...
       if ( ($data['resource'] == 'empty') || ($data['resource'] == 'new') )
            $errors['resource'] = get_string('error_resourcereq', 'liveclassroom');
