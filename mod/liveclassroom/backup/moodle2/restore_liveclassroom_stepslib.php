...
class restore_liveclassroom_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function process_liveclassroom($data) {
        ...
        $userinfo = $this->get_setting_value('userinfo');
        $status = true;
        $lcAction = new WIMBA_LCAction(null,$CFG->liveclassroom_servername,
        ...
        if (empty($sameResource)) {
            if (!$new_lc_id = $lcAction->WIMBA_cloneRoom($data->course, $data->type,
            ...
            if (!empty($resource)) {
                if ($userinfo) {
                    $room = $lcAction->WIMBA_getRoom($new_lc_id);
                    $room->WIMBA_setLongname($room->WIMBA_getLongname()." with user data");
                    $lcAction->api->WIMBA_lcapi_modify_room($new_lc_id, $room->WIMBA_getAttributes());
                } else {
                    $room = $lcAction->WIMBA_getRoom($resource->type);
                    $room->WIMBA_setLongname($room->WIMBA_getLongname()." without user data");
                    $lcAction->api->WIMBA_lcapi_modify_room($resource->type, $room->WIMBA_getAttributes());
                }
            }
        } else {
            $new_lc_id = $sameResource->type;
        }
        $data->isfirst = 1;
