<?php
    ...
    $prefix = $prefixUtil->WIMBA_getPrefix($user);
    $api = WIMBA_LCApi::WIMBA_getInstance($server, $user, $pass,$prefix, $PAGE->course->id, $CFG->dataroot);
    $api->WIMBA_lcapi_invalidate_auth();
    $auth = $api->WIMBA_lcapi_authenticate();

    if ($api->WIMBA_lcapi_get_error() != '' || $auth === false)
    {
