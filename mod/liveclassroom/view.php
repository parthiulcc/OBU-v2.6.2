...
if ((isset($action) && $action != "launchCalendar" && $id) || !$action)
{
  ...
  {
    print_error("This Wimba Classroom instance doesn't exist");
  }
} 
else 
{
...
$api = new WIMBA_LCAction(null, $CFG->liveclassroom_servername, $CFG->liveclassroom_adminusername, $CFG->liveclassroom_adminpassword, $CFG->dataroot,$liveclassroom->course);
...
#if(WIMBA_getRoleForWimbaTools($course->id, $USER->id)=="Instructor")
if (liveclassroom_getRole($context) == "Instructor")
{
	$authToken = $api->WIMBA_getAuthokenNormal($course->id."_T",$USER->firstname,$USER->lastname);
}
else
{
	$authToken = $api->WIMBA_getAuthokenNormal($course->id."_S",$USER->firstname,$USER->lastname);
...
//get the room
if(!$room=$api->WIMBA_getRoom($classid)) {
...
function WIMBA_startLiveClassroom() {
  startHorizon('<?php p($classid) ?>',null,null,null,null,'hzA=<?php p($authToken) ?>&<?php echo($api->api->WIMBA_get_bridge_header_string()) ?>'); 
}
</script>
    ...
    <div class="headerBar">
     ...
     <div style="height:340px;width:700px;">
        <span style="display:block;padding-top:150px;padding-left:200px">
                	<?php if($room->WIMBA_isPreview() == false || liveclassroom_getRole($context) == "Instructor"){?>
	                	<script>WIMBA_startLiveClassroom()</script>
	                   <?php echo get_string ('lcpopupshouldappear.1', 'liveclassroom');?>
					    <a href="javascript:WIMBA_startLiveClassroom ();">
