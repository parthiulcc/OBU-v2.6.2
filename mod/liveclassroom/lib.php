...
define("LIVECLASSROOM_MODULE_VERSION", "5.2.0-9");
if (!defined('WC'))
 ...
 * 
 * TODO: this method is supposed to be called by moodle when settings for module are saved
 *       but this isn't working - need to determine if moodle api spec has changed.
...
function liveclassroom_process_options (&$config)
{
    ...
    ********/
    ...
    if ($config->servername {$length-1} == '/') 
    {
        ...
        print_error(get_String('trailingSlash', 'liveclassroom'));
    } 
    ...
    if (!preg_match('/^http:\/\//', $config->servername) && !preg_match('/^https:\/\//', $config->servername))
    {
        ...
        print_error(get_String('trailingHttp', 'liveclassroom'));
    } 
    $prefixUtil = new WIMBA_PrefixUtil();
    $prefix = $prefixUtil->WIMBA_getPrefix($config->adminusername);
    $api = new WIMBA_LCApi($config->servername,
        $config->adminusername,
        $config->adminpassword, $prefix, $COURSE->id);

    $api->WIMBA_lcapi_invalidate_auth();
    if (! $api->WIMBA_lcapi_authenticate()) 
    {
        if($api->WIMBA_lcapi_get_error() == LCAPI_EADDR) {
            wimba_add_log(WIMBA_ERROR,WC,get_string('wrongconfigurationURLincorrect', 'liveclassroom'));
            print_error(get_string('wrongconfigurationURLincorrect', 'liveclassroom'));
        } else if ($api->WIMBA_lcapi_get_error() == LCAPI_EAUTH) {
            wimba_add_log(WIMBA_ERROR,WC,get_string('wrongadminpass', 'liveclassroom'));
            print_error(get_string('wrongadminpass', 'liveclassroom'));
        } else if($api->WIMBA_lcapi_get_error() == LCAPI_EHTTPS) {
            wimba_add_log(WIMBA_ERROR,WC,get_string('httpsnotenabled', 'liveclassroom'));
            print_error(get_string('configfailed','liveclassroom').': '.get_string('httpsnotenabled', 'liveclassroom'));
        } else if($api->WIMBA_lcapi_get_error() == LCAPI_EHTTP) {
            wimba_add_log(WIMBA_ERROR,WC,get_string('httpsrequired', 'liveclassroom'));
            print_error(get_string('configfailed','liveclassroom').': '.get_string('httpsrequired', 'liveclassroom'));
        } else if ($api->WIMBA_lcapi_get_error()) {
    ...
    if($domxml === false)
    {
        ...
        print_error(get_string('domxml', 'liveclassroom'));
    }
    return;
} 
...
function liveclassroom_add_instance($liveclassroom)
{
    ...
    $roomname = $api->WIMBA_getRoomName($liveclassroom->type); 
...
function liveclassroom_delete_all_instance_of_room($roomid,$prefix)
{
    ...
    // / this function will permanently delete the instance
    // / and any data that depends on it.
    $api = new WIMBA_LCApi($CFG->liveclassroom_servername,
                     $CFG->liveclassroom_adminusername,
    ...
    {
        $roomname = $api->WIMBA_lcapi_get_room_name($liveclassroom->type); 
        // Delete any dependent records here #
...
function liveclassroom_WIMBA_get_url_params($courseid)
{
    global $USER;
    global $CFG;
    ...
    wimba_add_log(WIMBA_DEBUG,WC,__FUNCTION__ . ": ENCODED courseid=".WIMBA_wimbaEncode($courseid)." - email=".WIMBA_wimbaEncode($email)." - firstname=".WIMBA_wimbaEncode($firstname)." - lastname=".WIMBA_wimbaEncode($lastname)." - role=".WIMBA_wimbaEncode($role));
     wimba_add_log(WIMBA_DEBUG,WC,__FUNCTION__ . ": signature=$signature");
    $url_params = "enc_course_id=" . WIMBA_wimbaEncode($courseid) . "&enc_email=" . WIMBA_wimbaEncode($email) . "&enc_firstname=" . WIMBA_wimbaEncode($firstname) . "&enc_lastname=" . WIMBA_wimbaEncode($lastname) . "&enc_role=" . WIMBA_wimbaEncode($role) . "&signature=" . WIMBA_wimbaEncode($signature);
    return $url_params;
}
...
*/
 function liveclassroom_reset_course_form($course) {
    ...
    {
         echo "There is no Wimba Classroom in this course";  
    }
}
...
function liveclassroom_delete_userdata($data, $showfeedback=true) {
   ...
   {
       $api = new WIMBA_LCAction(null,$CFG->liveclassroom_servername,
                        $CFG->liveclassroom_adminusername,
                        $CFG->liveclassroom_adminpassword,$CFG->dataroot,$data->id); 
       $rooms=$api->WIMBA_getRooms($data->id."_T");
      
       foreach ($rooms as $room)
       {
           if($room->WIMBA_isArchive() == 0)
           { 
                $isAdmin=$api->WIMBA_isStudentAdmin($room->WIMBA_getRoomId(), $data->id."_S");
                $api->WIMBA_cloneRoom($data->id,$room->WIMBA_getRoomId(),"0",$isAdmin,$room->WIMBA_isPreview());
                if($isAdmin == "true")
                {    
                    $api->WIMBA_removeRole($room->WIMBA_getRoomId(), $data->id."_S", "Student");
                    $api->WIMBA_removeRole($room->WIMBA_getRoomId(), $data->id."_T", "ClassAdmin");
                }
                else
                {
                    $api->WIMBA_removeRole($room->WIMBA_getRoomId(), $data->id."_S", "Instructor");
                    $api->WIMBA_removeRole($room->WIMBA_getRoomId(), $data->id."_T", "ClassAdmin");
                }
           }
           else
           {
                $api->WIMBA_deleteRoom($room->WIMBA_getRoomId());
           }
           $activities = $DB->get_records("liveclassroom", array("id" => $room->WIMBA_getRoomId()));
