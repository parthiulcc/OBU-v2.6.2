...
redirect($CFG->wwwroot."/mod/voicepresentation/mod.php$getstr");*/
...
$PAGE->requires->js('/mod/voicepresentation/lib/web/js/lib/prototype/prototype.js');
$PAGE->requires->js('/mod/voicepresentation/lib/web/js/wimba_ajax.js');
$PAGE->requires->js('/mod/voicepresentation/js/mod.js');
$PAGE->requires->js('/mod/voicepresentation/js/init_calendar_checkbox.js');

class mod_voicepresentation_mod_form extends moodleform_mod {
    ...
    function definition() {
        ...
        if(isset($this->current->update))
        {
            ...
            $activity = $DB->get_record("voicepresentation", array("id" => $cmVT->instance ));
        ...
        $url_params = voicepresentation_WIMBA_get_url_params($COURSE->id);
        ...
        //name
        $mform->addElement('text', 'name', get_string('activity_name', 'voicepresentation'), array('maxlength' =>255));
        ...
        $resopts['new'] = 'New Voice Presentation....';
        $resselc = isset($this->current->rid) ? $this->current->rid : 'empty';
        $mform->addElement('select', 'resource', get_string('voicepresentationtype', 'voicepresentation'), $resopts, array('onchange' => 'LoadNewFeaturePopup(this.options[this.selectedIndex].value);'));
        $mform->addRule('resource', null, 'required', null, 'client');
        $mform->setDefault('resource', $resselc);

        //calendar
        $mform->addElement('checkbox', 'calendar_event', get_string('add_calendar', 'voicepresentation'), '', array('onclick' => 'hideCalendarEvent("check");', 'id' => 'id_calendar_event'));
        ...
        if ($COURSE->format == 'weeks' || $COURSE->format == 'weekscss') {
            ...
            $mform->addElement('date', 'calendar', get_string('start_date', 'voicepresentation'), $options);
        } else {
        ...
        $mform->addGroup($durarr, null, get_string('duration_calendar', 'voicepresentation'), array(' '), false);
        ...
        $mform->addElement('textarea', 'description', get_string('description_calendar', 'voicepresentation'), $descopts);
    ...
    <div class="wimba_boxTop">
	    <div class="wimbaBoxTopDiv">
	            <span class="wimba_boxTopTitle"  style="width:300px;">'."Please enter a title for the new Voice Presentation :".'
	            </span>
	            <p>
   ...
   <div class="wimba_boxTop">
      <div class="wimbaBoxTopDiv">  
        <img src="'.$CFG->wwwroot.'/mod/voicepresentation/lib/web/pictures/items/wheel-24.gif"><br>
...
// buttons
    ...
    function validation($data, $files) {
        ...
        if ( ($data['resource'] == 'empty') || ($data['resource'] == 'new') )
            $errors['resource'] = get_string('resourcereq', 'voicepresentation');
