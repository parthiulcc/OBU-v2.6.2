...
function voicetools_api_isConfigured() {
  global $CFG;

  if ($CFG->voicetools_adminusername == '' &&
...
function voicetools_api_create_resource ($resource_data)
{
    ...
    try {
        ...
        wimba_add_log (WIMBA_ERROR, voicepresentation_LOGS, __FUNCTION__ . ' : Error to create the soap element : ' . $e->faultstring);
        return false;
    }
    ...
    try {
        ...
        $result = WIMBA_object_to_array($resp);
    } catch (Exception $e) {
        wimba_add_log (WIMBA_ERROR, voicepresentation_LOGS, __FUNCTION__ . ' : Fault with the web service : ' . $e->faultstring);
        return false;
    }
    $resource = new WIMBA_vtResource($result);

    if ($resource->error == "error") 
    {
        wimba_add_log (WIMBA_ERROR, voicepresentation_LOGS, __FUNCTION__ . " : Resource not created because " . $resource->error_message);
        return false;
    } 
    
    wimba_add_log (WIMBA_DEBUG, voicepresentation_LOGS, __FUNCTION__ . " : Resource Created");
    return $resource;
} 
...
function voicetools_api_modify_resource ($resource_data)
{
    ...
    if ($resource->error == "error") 
    {
        wimba_add_log (WIMBA_ERROR, voicepresentation_LOGS, __FUNCTION__ . " : Resource not modified because " . $resource->error_message);
        return false;
    } 
    
    wimba_add_log (WIMBA_DEBUG, voicepresentation_LOGS, __FUNCTION__ . " : Resource Modified");
    return $resource;
} 
...
function voicetools_api_delete_resource ($rid)
{
    ...
    if ($resource->error == "error") 
    {
        wimba_add_log (WIMBA_ERROR, voicepresentation_LOGS, __FUNCTION__ . " : Resource not deleted because " . $resource->error_message);
        return false;
    } 

    wimba_add_log (WIMBA_DEBUG, voicepresentation_LOGS, __FUNCTION__ . " : Resource Deleted");
    return $resource;
} 
...
function voicetools_api_get_resource($rid)
{
    ...
    if ($resource->error == "error") 
    {
        wimba_add_log (WIMBA_ERROR, voicepresentation_LOGS, __FUNCTION__ . " : Impossible to get the resource (rid=" . $rid . ") because " . $resource->error_message);
        return false;
    } 
    
    wimba_add_log (WIMBA_DEBUG, voicepresentation_LOGS, __FUNCTION__ . " : No problem to get the resource " . $rid);
    return $resource;
} 
...
function voicetools_api_get_resources($ridtable)
{
    ...
    $resources = new WIMBA_vtResources($result);

    if ($resources->error == "error") 
    {
        wimba_add_log (WIMBA_ERROR, voicepresentation_LOGS, __FUNCTION__ . " : Impossible to get the resources " . "because " . $resources->error_message);
        return false;
    } 
    
    wimba_add_log (WIMBA_DEBUG, voicepresentation_LOGS, __FUNCTION__ . " : No problem to get the resources " . print_r($ridtable,true));
    return $resources;
} 
...
function voicetools_api_create_session ($user, $ressource, $rights, $message = null)
{
    ...
    $session = new WIMBA_vtSessionInfo($result);
    if ($session->error == "error") 
    {
        wimba_add_log (WIMBA_ERROR, voicepresentation_LOGS, __FUNCTION__ . " : Impossible to create the session because " . $resources->error_message);
        return false;
    } 

    wimba_add_log (WIMBA_DEBUG, voicepresentation_LOGS, __FUNCTION__ . " : Session created ");
    return $session;
} 
...
function voicetools_api_check_documentbase($server, $login, $password, $url)
{
    ...
    try {
        ...
        wimba_add_log (WIMBA_ERROR, voicepresentation_LOGS, __FUNCTION__ . ' : Error to create the soap element');
...
function voicetools_api_check_result ($result)
{
    if (empty($result)) {
        ...
        wimba_add_log(WIMBA_ERROR,voicepresentation_LOGS,"voicetool_api_check_result: Empty result after call to " . VT_API_SERVICES);
        return false;
    } 

    if (voicetools_api_get_status_code($result) != 'ok') {
        wimba_add_log(WIMBA_ERROR,voicepresentation_LOGS,"voicetool_api_check_result: " . voicetools_api_get_error_message($result));
        return false;
    } 
    return true;
} 
...
function voicetools_get_nb_messages_per_user ($rid)
{
    ...
    return WIMBA_pairsetToArray($result); 
} 
...
function voicetools_get_average_length_messages_per_user ($rid)
{
 ...
 /**
   ...
   * @return an array.
   */
function WIMBA_pairsetToArray($pairset){
        ...
        if($groups != null) {
            for ($i = 0; $i < count($groups); $i++) 
            {
                if ($groups[$i] != null && $groups[$i]["pairSet"] != null && $groups[$i]["name"] != null) {
                  $hparams[$groups[$i]["name"]] = WIMBA_pairsetToArray($groups[$i]["pairSet"]);
                }
            }
        }
        if ($values != null) {
  ...
  * @return an array
  */
function WIMBA_object_to_array($object)
{
    ...
    return array_map('WIMBA_object_to_array', $object);
}
?>
