...
function launchSimpleAjaxRequest(url, action, init, div) {
    ...
    /*new Ajax.Request(urlXml,
    {
        ...
        onSuccess: function(transport){      
          xmlSource = transport;
          WIMBA_createSessionByXmlDocument(transport.responseXML);
    ...
    new Ajax.Request(urlXml,
    {
        ...
        onSuccess: function(transport){      
         ...
         new Ajax.Request(pathXsl,
         {
            ...
            mimetype:"application/xml",
            onSuccess: function (transport) {
                   var xslDoc = transport.responseXML;
                   if (transport.responseXML.documentElement == null) {
                     if (window.DOMParser) {
                       var parser=new DOMParser();
                       xslDoc=parser.parseFromString(transport.responseText,"text/xml");
                     } else { // Internet Explorer
                       xslDoc=new ActiveXObject("Microsoft.XMLDOM");
                       xslDoc.async=false;
                       xslDoc.loadXML(transport.responseText);
                     }
                   }
                   xsltTransformation(xmlSource,xslDoc,div,nextAction);
         }});    
        }
    });
}
...
function xsltTransformation(xml,xsl,div,nextAction){
    ...
    {   
        try { // IE9 and grater
            if (typeof (xml.responseXML.transformNode) != "undefined") { // IE6, IE7, IE8, & IE9
                ...
                if (window.ActiveXObject) {
                    ...
                    xslProc.input = xml.responseXML;
                    xslProc.transform();
                    var result = xslProc.output;
                }
            }
        } catch (e) {
            alert("Error transforming XML: "+e.message);
        }
        if (div == "all") 
        {
