...
function WIMBA_isInternetExplorer() {
...
function WIMBA_wimbaOnClick(element,onclickFunction)
{
 ...
 */
function WIMBA_searchResource(searchValue) {
                ...
                else if( allElements[i].getAttribute("name").toLowerCase().match(searchValue.toLowerCase()) )
                {
                    ...
                    {           
                        ...
                        if($(currentId).hasClassName( "grade:true" )){
                        	WIMBA_toggleToolBar("disabled",true);//the current element can be graded
                        }else{
                        	WIMBA_toggleToolBar("disabled",false);
                        }
                    }
...
*/
function WIMBA_clickElement(element, product, type, gradeId, linkedActivities) {

    linkedActivities = typeof linkedActivities !== 'undefined' ? linkedActivities : "";
    ...
    $(element).addLinkedActivities(linkedActivities);
    ...
    if( element != "" )
    {
        WIMBA_toggleToolBar("disabled");  
    }
    if(currentProduct == "voicetools" && type=="board"){
    	WIMBA_manageGradeButton();
    }
}

function WIMBA_manageGradeButton(){
 ...
 * 
 */
function WIMBA_hideArchive(element) {
...
*/
function WIMBA_changeToolBarView(typeView) {
    ...
    for (var x = 0; x < buttons.length; x++) {
        ...
        else
        {
            ...
            buttons[x].style.visibility="";
            WIMBA_initButton(buttons[x]);
        }
    }
}

function WIMBA_initButton(button) {
...
function WIMBA_switchView() {
    ...
    //WIMBA_changeFilterStatus(currentFilter, "disabled");
    //WIMBA_changeFilterStatus("filter_all", "enabled");
    //currentFilter = "filter_all";
    //init search
    $("searchField").value = "";
    WIMBA_searchResource("");//init the list
    WIMBA_changeToolBarView($("view").value);
    var allElements = $A($("list").getElementsByTagName("tr")).map(Element.extend);
...
function WIMBA_activateFilter(product) {
    ...
    WIMBA_changeFilterStatus(currentFilter, "disabled");
    WIMBA_changeFilterStatus("filter_" + product, "enabled");
...
*/
function WIMBA_onFilter(id) {
    if (currentFilter != id) {
        WIMBA_changeFilterStatus(id, "rollover");
    }
}

function WIMBA_outFilter(id) {
    if (currentFilter == id) {
        WIMBA_changeFilterStatus(id, "enabled");
    } else {
        WIMBA_changeFilterStatus(id, "disabled");
    }
}

function WIMBA_changeFilterStatus(id, status) {
    if ($(id) != null) {
        $(id).className = "filter" + status;
    }
}

function WIMBA_gestionDisplay(product) {
    var more = "more";
        ...
        for( var i = 0; i < products.length; i++ ) {
           if(products[i].getAttribute("typeElement")=="productList" && (product==products[i].getAttribute("product") || product==null)){
                ...
                if(numElement > 5 && product==null && products.length>4){
                    ...
                    divProductMore.innerHTML = "<label class='moreRoom' onclick=\"WIMBA_displayAllDiv('"+products[i].id+"','div_"+products[i].getAttribute("typeProduct")+"_More','"+numElement+"')\">" + (numElement - 5) + " "+more+"...</label>";
 ...
 */
function WIMBA_displayAllDiv(name, nameMore, number) {
    if (currentId != "") {
        $(currentId).style.backgroundColor = "white";
    }
    currentId = "";
    ...
    expandState = 1;
    $(name).style.height = "";
    $(nameMore).innerHTML = "<label class='moreRoom' onclick='WIMBA_RemoveAllDiv(\"" + name + "\",\"" + nameMore + "\",\"" + number + "\")'>Show Top 5...</label>";
}
/*
 * Called to hide the complete list of element and display just 5 elements
 */
function WIMBA_RemoveAllDiv(name, nameMore, number) {
    if (currentId != "") {
        $(currentId).style.backgroundColor = "white";
    }
    currentId = "";
    ...
    $(nameMore).innerHTML = "<label class='moreRoom' onclick='WIMBA_displayAllDiv(\"" + name + "\",\"" + nameMore + "\",\"" + number + "\")'>" + (number - 5) + " more...</label>";
}

/*
 * Called to hide the complete list of elements
 */
function WIMBA_collapseGroupElement(product, pictureId) {
 ...
 */
function WIMBA_createSessionByXmlDocument(data) {
 ...
 */
function WIMBA_LaunchElement(url) {
    if ( currentProduct == LC_PRODUCT && currentId != "") 
    {
        WIMBA_LaunchLiveClassroom(url);
    } 
    else 
    {
        if ( currentProduct == VT_PRODUCT  && currentId != "") 
        {
            WIMBA_LaunchVoiceTools(url);
        }
    }
}
 ...
 */
function WIMBA_LaunchLiveClassroom(url, params) {
    var result = true;
 ...
 */
function WIMBA_LaunchVoiceTools(url, params) {
    var result = true;
    //Student view
    if (session["role"] == "StudentBis" || (session["role"] == "Instructor" && studentView == true)) {
        result = window.confirm("You are in Student View.\nYou will not have access to instructor features.");
 ...
 */
function WIMBA_onTab(id, additionalfunction) 
{
    if( !$("tab" + id).hasClassName('disabled'))
    {
        ...
        else 
        {
            ...
            {//click on the advanced tab
                ...
                $("advancedOk").onclick=function(){WIMBA_gotoAdvanced(currentIdtab , id , additionalfunction );};
                $("advancedPopup").show();
            } 
            else 
            {
 ...
 */
function WIMBA_toggleTypeOfRoom(type) {
...
var last_state = true;
function WIMBA_managePublicState(id, type) {
    if (type == "private") {
        $(id).disabled = true;
...
function WIMBA_managePointsPossible(){
...
*/
function WIMBA_submitForm(url, action, id) {
...
function WIMBA_submitGradeForm(url, action, params){
	if(WIMBA_checkGrade()){
...
function WIMBA_redirectToActivity(url, action, id){
 ...
 */
function WIMBA_closeMessageBar() {
    $("message").hide();
}
/*
 * called when you clik on settings
 */
function WIMBA_editSettings(url, div) {
    if (currentId != "") {
 ...
 */
function WIMBA_openReport(url) {
    if (currentId != "" && currentProduct == LC_PRODUCT) {
        WIMBA_doOpen(url + "?action=openReport&", true);
    }
}
/*
 * called when you clik on delete
 */
function WIMBA_deleteResource(url) {
    if ($(currentId).getLinkedActivities() != "") {
        var list = "";
        list = $(currentId).getLinkedActivities();
        alert("We're sorry - this object is attached to an Activity. Please first delete the Activity:\n" + list.replace('|','\n'));
        return;
    }
    if (currentId != "" && WIMBA_confirmDelete()) {
        WIMBA_doOpen(url + "?action=delete&", false);
    }
}
/*
 * called when you clik on content
 */
function WIMBA_openContent(url) {
    if (currentId != "" && currentProduct == LC_PRODUCT) {
        WIMBA_doOpen(url + "?action=openContent&", true);
    }
}
/*
 * ask for a confirmation before deleting something
 */
function WIMBA_confirmDelete()
{
    var type = ""
    if( currentProduct == LC_PRODUCT )
    {
...
function WIMBA_doOpen(url, popup) {
...
function WIMBA_doOpenPodcaster(url, popup) {
...
*/
function WIMBA_gotoAdvanced(currentId, id, additionalFunction) {
...
*/
function WIMBA_openRoomSettings(url) {
...
*/
function WIMBA_openMediaSettings(url) {
...
function WIMBA_manageAvailibility() {
    ...
    WIMBA_manageAvailibilityDate("start", disabled);
    WIMBA_manageAvailibilityDate("end", disabled);
}

/*
 * manage the availability of the date fields
 */
function WIMBA_manageAvailibilityDate(type, disabled) {
...
function WIMBA_convertDate(str) {
 ...
 * Display a rss feed
 */
function WIMBA_displayRssFeed(data, div) {
    ...
    $(div).innerHTML = WIMBA_displayItem(channel);
}

//display a feed item
function WIMBA_displayItem(elementParameters) {
    var length = 0;
    var display = "";
    //Get items
    ...
    if (item.length == 0) {
        display += "<tr>";
        display += "<td class=noPodcaster>The Voice Podcaster for this course is empty. To make a recording, click the link below to open the Voice Podcaster and create a new post.</td>";
        display += "</tr>";
    } else {
        if (item.length >= 5) {
            length = 5;
        ...
        for (var i = 0; i < length; i++) {
            ...
            display += "<td>" + WIMBA_convertDate(item[i].getElements("pubDate")[0].getText()) + "</td>";
            display += "<td align=right style='padding-right:10px' class='link' onclick='javascript:WIMBA_doOpenPodcaster(\"" + item[i].getElements("link")[0].getText() + "\",true)'>Read & Listen</td>";
            display += "</tr>";
        }
    }
    display += "</table>";
...
function WIMBA_launchPodcast(url) {
    if (currentId != "") {
        WIMBA_doOpen(url + "?action=launch&", true);
    }
}
...
function WIMBA_addLoadEvent(func) {
...
function WIMBA_init() {
    $("searchField").onkeyup = function() {
        WIMBA_searchResource(this.value);
        WIMBA_displaySearchFieldResetBtn(this.value);
    }       
    ...
    $("searchFieldResetBtn").onclick = function() {
        WIMBA_resetSearchField();
        WIMBA_searchResource("");
    }       
}

function WIMBA_detectSafari() {
...
function WIMBA_displaySearchFieldResetBtn(s) {
    if (s.length > 0) {
...
function WIMBA_resetSearchField() {
    $('searchField').value = "";
    WIMBA_displaySearchFieldResetBtn("");
}   

function WIMBA_showGrades(url){
...
function WIMBA_doChangeMediaPriority()
{
...
/*
 * Sync Enable Archiving on Features tab with Enable Archives on Archives tab
 */
function WIMBA_doToggleArchivesStatus(archiveEnabled, enable_archives) {
  archiveEnabled.checked = enable_archives.checked;
}

function WIMBA_doToggleFeaturesArchiving(archiveEnabled, enable_archives) {
  enable_archives.checked = archiveEnabled.checked;
}

function submitenter(myfield,e,url,action,rid)
{
  var keycode;
  if (window.event) {
  ...
  if (keycode == 13) {
    WIMBA_submitForm(url, action, rid);
    return false;
  } else {
    return true;
  }
