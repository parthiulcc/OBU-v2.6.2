...
function create(name,courseid){
    if (name == '')
        return false;
    createNewResource(M.cfg.wwwroot+"/mod/voicepresentation/manageAction.php","voicetools","presentation",name,$F($('mform1')["url_params"]));
}

function LoadNewFeaturePopup(current)
{
    if( current == "new" ){
        var ret = prompt('Please enter a title for the new Voice Presentation');
        var pattern=/^\s+$/g;
        if (pattern.test(ret)) {
          $('id_resource').selectedIndex=0;
          alert('Voice Presentation name can not be blank');
          return false;
        }
        if (ret == null) {
          $('id_resource').selectedIndex=0;
