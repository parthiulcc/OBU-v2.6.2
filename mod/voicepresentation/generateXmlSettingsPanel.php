...
if (!voicetools_api_isConfigured()) {
    $uiManager->WIMBA_setError(get_string ('error_unconfigured_vt', 'voicepresentation'));
}
...
if($uiManager->WIMBA_getSessionError() === false)
{
    ...
    { //get the information of the resource
        ...
        if( !isset($currentBoard) || $currentBoard->error==true)
        {
            ...
            $uiManager->WIMBA_setError(get_string('problem_vt','voicepresentation'));
        }
        else
        {
...
else
{
    ...
    $uiManager->WIMBA_setError(get_string ('error_session', 'voicepresentation'));
}
wimba_add_log(WIMBA_DEBUG,voicepresentation_LOGS,"getXmlListPanel : parameters  \n" .  $uiManager->WIMBA_getXmlString()); 

if(isset($error_wimba))//error fatal was detected
{
    $uiManager->WIMBA_setError(get_string ('error_display', 'voicepresentation'));
}
