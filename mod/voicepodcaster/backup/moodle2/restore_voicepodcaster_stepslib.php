...
class restore_voicepodcaster_activity_structure_step extends restore_activity_structure_step {
    ...
    // Also see restore_voicepodcaster_activity_task::after_restore for additional grade processing on restore
    protected function process_voicepodcaster($data) {
        ...
        if($userinfo)
        {
            ...
            $resourceDbMatched = $DB->get_record("voicepodcaster_resources", array("fromrid" => $old_rid, "course" => $data->course, "copyoptions" => $typeCopy)); //resource which match the current copy options
            $resourceDbOther = $DB->get_record("voicepodcaster_resources", array("fromrid" => $old_rid, "course" => $data->course, "copyoptions" => "2"));
        }
        else
        {
            $copyOptions = "2";//delete all
            $resourceDbMatched = $DB->get_record("voicepodcaster_resources", array("fromrid" => $old_rid, "course" => $data->course, "copyoptions" => "2"));
            $resourceDbOther = $DB->get_record("voicepodcaster_resources", array("fromrid" => $old_rid, "course" => $data->course, "copyoptions" => $typeCopy));
        }
        ...
        if($isGradable == 'true') { //the old vb was gradable
            ...
            if(isset($actionGradebook) && $actionGradebook == "update") {
                ...
                voicepodcaster_add_grade_column($newResource->WIMBA_getRid(), $this->get_courseid(), $newResource->WIMBA_getTitle(), $newResourceCopyOptions->WIMBA_getPointsPossible());
