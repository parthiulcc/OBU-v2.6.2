...
redirect($CFG->wwwroot."/mod/voicepodcaster/mod.php$getstr");*/
...
$PAGE->requires->js('/mod/voicepodcaster/lib/web/js/lib/prototype/prototype.js');
$PAGE->requires->js('/mod/voicepodcaster/lib/web/js/wimba_ajax.js');
$PAGE->requires->js('/mod/voicepodcaster/js/mod.js');
$PAGE->requires->js('/mod/voicepodcaster/js/init_calendar_checkbox.js');

class mod_voicepodcaster_mod_form extends moodleform_mod {
    ...
    function definition() {
        ...
        if (!voicetools_api_isConfigured()) {
            print_error(get_string('error_unconfigured_vt','voicepodcaster'));
        }
        ...
        {
            $disabled="";
            $textSaveButton=get_string('validationElement_saveAllActivity', 'voicepodcaster');
            $textSaveAndBackButton=get_string('validationElement_saveAllAndBack', 'voicepodcaster');
            $isButtonDisabled = "";
            ...
            $activity = $DB->get_record("voicepodcaster", array("id" => $cmVT->instance ));
            $sectionId=$activity->section;
            $resource_id = $activity->rid;

            $event=voicepodcaster_get_event_calendar($activity->id);
            if( $event )
            {
            ...
            $name=$activity->name;
            $activity_context=get_string('updateActivity', 'voicepodcaster');
        }
        ...
        $resourcesRid= voicepodcaster_get_voicetools_list($COURSE->id);
        $vtResources=voicetools_api_get_resources($resourcesRid["rid"]);

        $url_params = voicepodcaster_WIMBA_get_url_params($COURSE->id);
        ...
        //name
        $mform->addElement('text', 'name', get_string('activity_name', 'voicepodcaster'), array('maxlength' =>255));
        $mform->addRule('name', null, 'required', null, 'client');

        $resopts = array('empty' => 'Select...');
        for($i=0;$vtResources!=null && $i<count($vtResources->WIMBA_getResources());$i++)
        {
            $resource = $vtResources->WIMBA_getResource($i);
            if (isset($resource_id) && $resource_id == $resource->WIMBA_getRid())
                $resselected = $resource->WIMBA_getRid();
            $resopts[$resource->WIMBA_getRid()] = ((strlen($resource->WIMBA_getTitle()) > 25)
                                            ? substr($resource->WIMBA_getTitle(),0,25)."..."
                                            : $resource->WIMBA_getTitle());
        }
        $resopts['new'] = 'New Voice Podcaster....';
        $resselc = isset($this->current->rid) ? $this->current->rid : 'empty';
        $mform->addElement('select', 'resource', get_string('voicepodcastertype', 'voicepodcaster'), $resopts, array('onchange' => 'LoadNewFeaturePopup(this.options[this.selectedIndex].value);'));
        $mform->addRule('resource', null, 'required', null, 'client');
        $mform->setDefault('resource', $resselc);

        //calendar
        $mform->addElement('checkbox', 'calendar_event', get_string('add_calendar', 'voicepodcaster'), '', array('onclick' => 'hideCalendarEvent("check");', 'id' => 'id_calendar_event'));
        ...
        if ($COURSE->format == 'weeks' || $COURSE->format == 'weekscss') {
            ...
            $mform->addElement('date', 'calendar', get_string('start_date', 'voicepodcaster'), $options);
        } else {
        ...
        foreach (range(0,60,10) as $min)
            $minarr[$min] = $min;
        $durarr = array();
        $durarr[] = $mform->createElement('select', 'duration_hrs', '', range(0, 23));
        $durarr[] = $mform->createElement('select', 'duration_min', '', $minarr);
        $mform->addGroup($durarr, null, get_string('duration_calendar', 'voicepodcaster'), array(' '), false);
        ...
        $mform->addElement('textarea', 'description', get_string('description_calendar', 'voicepodcaster'), $descopts);
    ...
    <div class="wimba_boxTop">
	    <div class="wimbaBoxTopDiv">
	            <span class="wimba_boxTopTitle"  style="width:300px;">'."Please enter a title for the new Voice Podcaster :".'
	            </span>
	            <p>
   ...
   <div class="wimba_boxTop">
      <div class="wimbaBoxTopDiv">  
        <img src="'.$CFG->wwwroot.'/mod/voicepodcaster/lib/web/pictures/items/wheel-24.gif"><br>
...
// buttons
    ...
    function validation($data, $files) {
        ...
        if ( ($data['resource'] == 'empty') || ($data['resource'] == 'new') )
            $errors['resource'] = get_string('resourcereq', 'voicepodcaster');
