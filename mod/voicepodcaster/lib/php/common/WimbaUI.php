    ...
    */    
    function WIMBA_getVTPrincipalView($message,$type=null) {
        ...
        $this->xml->WIMBA_addButtonElement(
                        ...
                        "javascript:WIMBA_LaunchElement('manageAction.php','voicepodcaster');");
        ...
        if($this->session->WIMBA_isInstructor())
        {
            $this->xml->WIMBA_addButtonElement(
                            ...
                            "WIMBA_doOpenAddActivity('../../course/mod.php','section=0&sesskey=" . sesskey() . "&add=voicepodcaster')");
        ...
        {//problem to get the vt resource
            ...
            $this->xml->WIMBA_setError(get_string("error_connection_vt", "voicepodcaster"));
        }
...
* ***********************************/
    ...
    */ 
    function WIMBA_createVPInfoPanel() {
        ...
        $this->xml->WIMBA_addInputElement($parameters);
        $this->xml->WIMBA_addSimpleLineElement("label", get_string('comment_slide', 'voicepodcaster'));
        ...
        $this->xml->WIMBA_addInputElement($parameters);
        $this->xml->WIMBA_addSimpleLineElement("label", get_string('private_slide', 'voicepodcaster'));
        $this->xml->WIMBA_createLine();
    ...
    */     
    function WIMBA_createVBFeaturesPanel() {
        ...
        $this->xml->WIMBA_addInputElement($parameters);
        $this->xml->WIMBA_addSimpleLineElement("label", get_string('short_message', 'voicepodcaster'));
        ...
        $this->xml->WIMBA_addInputElement($parameters);
        $this->xml->WIMBA_addSimpleLineElement("label", get_string('chrono_order', 'voicepodcaster'));
        ...
        $this->xml->WIMBA_addInputElement($parameters);
        $this->xml->WIMBA_addSimpleLineElement("label", get_string('show_forward', 'voicepodcaster'));
        ...
        $this->xml->WIMBA_addInputElement($parameters);
        $this->xml->WIMBA_addSimpleLineElement("label", get_string('settings_show_reply', 'voicepodcaster'));
        ...
        if(WIMBA_isGradebookAvailable()){
          ...
          if(isset ($this->currentObject)){
            $activities = $DB->get_records("voicepodcaster", array("rid" => $this->currentObject->WIMBA_getRid()));
            if(empty($activities)){
             $parameters['disabled'] = "true";
            }
          }else{
          ...
          $this->xml->WIMBA_addInputElement($parameters);
          $this->xml->WIMBA_addSimpleLineElement("label", get_string('grade_settings', 'voicepodcaster'));
    ...
    */   
	...
	global $DB;
        ...
        for ($i = 0; $i < count($ressources); $i++) 
        {
            ...
            if ( $voicepodcaster = $DB->get_records("voicepodcaster", array("rid" => $rid))) {
                 $activities = array();
            ...
            $xmlResource->WIMBA_setTooltipAvailability(get_string("tooltipVT_" . $preview . "_student", 'voicepodcaster'));
