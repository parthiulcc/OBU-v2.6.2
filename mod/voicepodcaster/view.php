...
if(voicepodcaster_isView_From_GradeBook() == TRUE){
  //If we're coming from the gradebook, direct user to grading page as opposed to voice tool page
    ...
    if (! $voicetool = $DB->get_record("voicepodcaster", array("id" => $cm->instance)))
...
$PAGE->set_url('/mod/voicepodcaster/view.php', array('id'=>$id));

$cm = get_coursemodule_from_instance("voicepodcaster", $voicetool->id, $course->id);
...
if ($voicetool->isfirst == 0)
{
    ...
    $DB->update_record("voicepodcaster",$voicetool);
...
$sentence2="<a href='javascript:WIMBA_startVoiceBoard ()';>".get_string ('vtpopupshouldappear.2', 'voicepodcaster')."</a>";
...
if($resource->error==true)
{
    ...
    print_error(get_string("problem_vt", "voicepodcaster"), "$CFG->wwwroot/course/view.php?id=$course->id");
}
