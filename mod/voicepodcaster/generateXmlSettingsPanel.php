...
if (!voicetools_api_isConfigured()) {
    $uiManager->WIMBA_setError(get_string ('error_unconfigured_vt', 'voicepodcaster'));
}
...
if($uiManager->WIMBA_getSessionError() === false)
{
    ...
    { //get the information of the resource
        ...
        if( !isset($currentBoard) || $currentBoard->error==true)
        {
            ...
            $uiManager->WIMBA_setError(get_string('problem_vt','voicepodcaster'));
        }
        else
        {
...
else
{
    ...
    $uiManager->WIMBA_setError(get_string ('error_session', 'voicepodcaster'));
}
wimba_add_log(WIMBA_DEBUG,voicepodcaster_LOGS,"getXmlListPanel : parameters  \n" .  $uiManager->WIMBA_getXmlString()); 

if(isset($error_wimba))//error fatal was detected
{
    $uiManager->WIMBA_setError(get_string ('error_display', 'voicepodcaster'));
}
