...
$PAGE->requires->js('/mod/voiceauthoring/js/init_calendar_checkbox.js');
...
class mod_voiceauthoring_mod_form extends moodleform_mod {
    ...
    function definition() {
        ...
        */
        ...
        $vtUser = new WIMBA_VtUser();
        $vtUserRigths = new WIMBA_vtRights();
        $vtUserRigths->WIMBA_setProfile ('moodle.recorder.instructor');
        $message = new WIMBA_vtMessage();

        if(!empty($voiceauthoring))
        {
            $rid_voiceAuthoring=$voiceauthoring->rid;
            $resource = $vtAction->WIMBA_getResource($rid_voiceAuthoring) ;
            if($resource==null)
            {
                $error=true;
            }
        ...
        { //creation of the voice authoring linked to this course
            ...
            $mid=0;
            $resource = $vtAction->WIMBA_createRecorder("Voice Authoring associated to the course ".$course_id) ;
            if ($resource === false)
            ...
            {
                $rid_voiceAuthoring = $resource->WIMBA_getRid();
                WIMBA_storeRecorderResource($rid_voiceAuthoring,$COURSE->id,$mid, "voiceauthoring");//default availability
                $message->WIMBA_setMid("va-".$mid);     
            }
        }
        ...
        $url_params = voiceauthoring_WIMBA_get_url_params($course_id);      
