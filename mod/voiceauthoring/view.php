...
//WIMBA_redirection("$CFG->wwwroot/course/view.php?id=$course->id#$voicetool->section");
$url = "displayPlayer.php?rid=".$voicetool->rid."&mid=".$voicetool->mid."&title=".urlencode($voicetool->activityname)."&cid=".$course->id;
if( !empty($action) &&  format_string ("<iframe>test</iframe>") != "test" )//iframe allowed
{
    WIMBA_redirection("$CFG->wwwroot/course/view.php?id=$course->id&launchCal=".$action);
}
else
{
...
$sentence2 = "<a href='javascript:WIMBA_openVA()';>".get_string ('vtpopupshouldappear.2', 'voiceauthoring')."</a>";
