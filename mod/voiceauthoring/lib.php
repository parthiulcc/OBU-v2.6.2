...
function voiceauthoring_add_instance($voicetool) {
  ...
  /// of the new instance.
    ...
    $iFramePath = $CFG->wwwroot."/mod/voiceauthoring/displayPlayer.php?rid=".$voicetool->rid."&mid="."va-".$voicetool->mid."&title=".urlencode($voicetool->name)."&cid=".$COURSE->id;
    $iFramePath1 = $CFG->wwwroot."/mod/voiceauthoring/displayActivityName.php?rid=".$voicetool->rid."&mid="."va-".$voicetool->mid."&title=".urlencode($voicetool->name)."&cid=".$COURSE->id;
    ...
    if ($resource = $DB->get_record("voiceauthoring_resources", array("rid" => $voicetool->rid)))
    {
        ...
        {    
            WIMBA_updateRecorderResource($voicetool->rid,$voicetool->course,$mid_number, "voiceauthoring");//update the mid for the next message
        }
    }
...
function voiceauthoring_update_instance($voicetool) {
  ...
  /// (defined by the form in mod.html) this function
    ...
    $objectPath = $CFG->wwwroot."/mod/voiceauthoring/enableVA.php";
    $imgPath = $CFG->wwwroot."/mod/voiceauthoring/lib/web/pictures/items/speaker-18.gif";
    $iFramePath = $CFG->wwwroot."/mod/voiceauthoring/displayPlayer.php?rid=".$voicetool->rid."&mid=".$voicetool->mid."&cid=".$voicetool->course;
    $iFramePath1 = $CFG->wwwroot."/mod/voiceauthoring/displayActivityName.php?rid=".$voicetool->rid."&mid=".$voicetool->mid."&title=".urlencode($voicetool->name)."&cid=".$voicetool->course;
    $voicetool->activityname=htmlspecialchars($voicetool->name, ENT_QUOTES);
    $voicetool->name=htmlspecialchars($voicetool->name, ENT_QUOTES);
...
function voiceauthoring_WIMBA_get_url_params($courseid)
{
    global $USER;
    global $CFG;
...
*/
 function voiceauthoring_reset_course_form($course) {
   ...
   /**
    * Actual implementation of the reset coures functionality, delete all the
...
function voiceauthoring_reset_userdata($data) {
   ...
   if (!empty($data->reset_content_voiceauthoring)) {
        ...
        $resource->rid = $newResource->WIMBA_getRid();
        ...
        {
            $new_rid = $newResource->WIMBA_getRid();
