...
$cid=optional_param('cid','',PARAM_CLEAN);

//Require login
require_login($cid);

$imgPath = $CFG->wwwroot."/mod/voiceauthoring/lib/web/pictures/items/speaker-18.gif";
$iFramePath = $CFG->wwwroot."/mod/voiceauthoring/displayPlayer.php?rid=".$rid."&mid=".$mid."&title=".urlencode($title)."&cid=".$cid;
?>
...
function WIMBA_displayRecorder()
{
...
function WIMBA_displayRecorderForCalendar()
{     
...
<body  style="padding-top:4px;" id="bodyPlayer">
   <div style="padding-left:2px">     
        <img width='16px' height='16px' onclick="WIMBA_displayRecorder()" style="cursor:pointer" src='<?php echo $imgPath?>'/>
    </div>

<script>
    WIMBA_displayRecorderForCalendar();
    var span=parent.document.getElementById('<?php echo $rid?>_<?php echo $mid?>_span');
    if (span.addEventListener) {
            // EOMB
        span.addEventListener('click',function(e){
             WIMBA_displayRecorder();
             e.preventDefault();            
        },true);
    } else {
            // IE
        span.attachEvent('onclick',WIMBA_displayRecorder);
    }
