...
class WIMBA_LCApi {
    ...
    function WIMBA_LCApi($server, $login, $password, $prefix_api, $courseId, $tmppath = "")
    {
        global $CFG, $DB;
    ...
    function WIMBA_getAdminName()
    {
        return $this->_lcapi_config["login"];
    }

    public static function WIMBA_getInstance($server, $login, $password, $prefix, $courseId, $path)
    {
        static $myInstance = null;
        if ($myInstance == null) 
        {
            $myInstance = new WIMBA_LCApi($server, $login, $password, $prefix, $courseId, $path);
        } 
        return $myInstance;
    } 

    function WIMBA_isConfigured() {
        // http://devtools.bbbb.net:8080/browse/CVMI-179
        // We want to distinguish between a failed connection and an unconfigured block.
        // First, we will see if the URL, user, and pass are blank.
        if ($this->_lcapi_config["login"] == '' &&
            $this->_lcapi_config["password"] == '' &&
            $this->_lcapi_config["server"] == '') {
            return false;
        }
     ...
     * @return nothing 
     */
    function WIMBA_lcapi_error($error)
    {
        $this->error = $error;
    }
     ...
     */
    function WIMBA_lcapi_get_error()
    {
        return $this->error;
    }
     ...
     * @return the last error code
     */
    function WIMBA_lcapi_get_errormsg($error=null)
    {
      if ($error === null) {
        return $this->error_msg_map[$this->WIMBA_lcapi_get_error()];
      } else {
     ...
     */
    function WIMBA_get_bridge_header_elements() {
     ...
     */
    function WIMBA_get_bridge_header_string() {
      $headarr = $this->WIMBA_get_bridge_header_elements();
    ...
    function WIMBA_lcapi_authenticate() {
      ...
      if (!$this->WIMBA_lcapi_check_auth()) {
        ...
        while ($authInvalidated <= 1) {
          ...
          $respcode = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_NOOP,$params,true);
          ...
          if ($respcode == true) {
            ...
            if ($authInvalidated == 0) {
              ...
              $this->WIMBA_lcapi_invalidate_auth();
              $authInvalidated++;
            } else {
    ...
    function WIMBA_lcapi_check_auth() {
    ...
    function WIMBA_lcapi_invalidate_auth() {
        if(file_exists($this->tstampPath)) {
     ...
     */
    function WIMBA_lcapi_send_query($function, $params, $disable_maxredirs=false)
    {
        global $CFG;
        
        wimba_add_log(WIMBA_DEBUG,WC,__FUNCTION__ . ": Executing function=".$function);

        if($function != LCAPI_FUNCTION_NOOP && $function != LCAPI_FUNCTION_STATUS && !$this->WIMBA_lcapi_check_auth()) {
            wimba_add_log(WIMBA_DEBUG,WC,__FUNCTION__ . ": not authenticated, trying");
            if (!$this->WIMBA_lcapi_authenticate()) {
              return false;
            }
        }
        ...
        $bridgeHeaderElements = $this->WIMBA_get_bridge_header_elements();
        ...
        if(preg_match('/HTTPS Not Supported/',$response)) {
            $this->WIMBA_lcapi_error(LCAPI_EHTTPS);
        ...
        if($errno > 0 && preg_match('/^http:\/\//i',$url)) {
            $this->WIMBA_lcapi_error(LCAPI_EHTTP);
        ...
        if(preg_match('/HTTP Status 401.*This request requires HTTP authentication/',$response)) {
            $this->WIMBA_lcapi_error(LCAPI_EAUTH);
        ...
        if(preg_match('/HTTP Status 500.*cluster status is not OK/',$response)) {
            $this->WIMBA_lcapi_error(LCAPI_EPBCON);
        ...
        if($function == LCAPI_FUNCTION_NOOP) {
            if (isset($response)) {
                ...
                $this->WIMBA_lcapi_error(LCAPI_ETIME);
     ...
     */
    function WIMBA_lcapi_create_user($userid, $firstname="", $lastname="")
    {
     ...
     * false otherwise.
     */
    function WIMBA_lcapi_get_session($userid, $nickname)
    {
    wimba_add_log(WIMBA_ERROR,WC,__FUNCTION__ . " IN lcapi_get_session");
        ...
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_GET_TOKEN, $enc_params);

        preg_match("(\d*)", $response, $matches);
        $respcode = $matches[0];
        if ($respcode != 100) 
        {
            $this->WIMBA_lcapi_error(LCAPI_ENOSESN);
     ...
     */
    function WIMBA_lcapi_check_room_params(&$attributes)
    {
     ...
     */
    function WIMBA_lcapi_create_class($roomid, $roomname, $attributes)
    {
        $params = '';
        $this->WIMBA_lcapi_check_room_params($attributes);
        ...
        $enc_params = str_replace(" ", "+", $params);
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_CREATE_CLASS, $enc_params);
        preg_match("(\d*)", $response, $matches);
        $respcode = $matches[0];
        if ($respcode == 301) 
        {
     ...
     */
    function WIMBA_lcapi_add_user_role($roomid, $userid, $role)
    {
        if ($userid != "Guest") {
        ...
        $params = "&target=".$this->prefix.$roomid .
                  "&user_id=".$userid.
                  "&role_id=".$role;;
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_CREATE_ROLE,$params);
        preg_match("(\d*)", $response, $matches);
        $respcode = $matches[0];
        if ($respcode != 100) 
        {
     ...
     */
    function WIMBA_lcapi_remove_user_role($roomid, $userid, $role)
    {
        if ($userid != "Guest") 
        {
            $userid = $this->prefix . $userid;
        ...
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_DELETE_ROLE, $params);
        preg_match("(\d*)", $response, $matches);
        $respcode = $matches[0];
        if ($respcode != 100) 
        {
     ...
     */
    function WIMBA_lcapi_delete_room($roomid)
    {
        $params = "&target=".$this->prefix.$roomid;
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_DELETE_ROOM, $params);
     ...
     */
    function WIMBA_lcapi_modify_room($roomid, $attributes)
    {
        $params = "&target=" . $this->prefix . $roomid;
        while (list ($key, $val) = each($attributes)) 
        {
        ...
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_MODIFY_ROOM, $params);
        preg_match("(\d*)", $response, $matches);
        $respcode = $matches[0];
        if ($respcode != 100) 
        {
     ...
     */
    function WIMBA_lcapi_get_archives($userid, $roomid)
    {
        ...
        $rooms = null;
        $enc_params = str_replace(" ", "+", $params);
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_GET_ROOM_LIST, $enc_params);
        ...
        for ($i = 0; $i < sizeof($records); $i++) 
        {
            ...
            if ((empty ($roomid) && !empty ($record)) || // orphaned archive or room
                ...
                $room->WIMBA_setByRecord($record, $this->prefix);
                $rooms[] = $room;
            } 
        } 
     ...
     * for all rooms this LC user has access to.
     */
    function WIMBA_lcapi_get_rooms($userid)
    {
        $params = "&attribute=".LCAPI_ATTR_PREVIEW.
                  "&attribute=".LCAPI_ATTR_ARCHIVE.
                  "&attribute=".LCAPI_ATTR_CLASSID.
        ...
        {
            if(!isset($this->error)) { $this->WIMBA_lcapi_error(LCAPI_ECRUD); }
     ...
     */
    function WIMBA_lcapi_get_users($userid = '', $firstname = '', $lastname = '')
    {
        ...
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_LIST_USER, $request);
     ...
     */
    function WIMBA_lcapi_get_user_role($roomid, $userid)
    {
        if ($userid != "Guest")
        {
            $userid = $this->prefix.$userid;
        ...
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_LIST_ROLE, $params);
        preg_match("(\d*)", $response, $matches);
        $respcode = $matches[0];
        if ($respcode != 100) 
        {
     ...
     */
    function WIMBA_lcapi_room_is_archive($roomid)
    {
        ...
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_GET_ROOM_LIST, $params);
        preg_match("(\d*)", $response, $matches);
        $respcode = $matches[0];
        if ($respcode != 100) 
        {
     ...
     */
    function WIMBA_lcapi_get_room_name($roomid)
    {
     ...
     */
    function WIMBA_lcapi_get_media_type($roomid)
    {
        $params = "&attribute=" . LCAPI_ATTR_MEDIATYPE . "&filter00=" . LCAPI_ATTR_CLASSID . "&filter00value=" . $this->prefix . $roomid;
        $response = WIMBA_lcapi_send_query(LCAPI_FUNCTION_GET_ROOM_LIST, $params);
        preg_match("(\d*)", $response, $matches);
        $respcode = $matches[0];
        if ($respcode != 100) 
        {
     ...
     */
    function WIMBA_lcapi_get_room_info($roomid)
    {
        $roominfo=array();
        ...
        if ($respcode != 100) 
        {
            // $this->WIMBA_lcapi_error(self::LCAPIEQRY);
        ...
        foreach ($lines as $line) 
        {
            if(!empty($line)){
                ...
                if (!empty ($key)) 
                {
                    // http://devtools.bbbb.net:8080/browse/CVMI-187
                    if ($key == "description") {
                        $roominfo[$key] = str_replace('\\n','&#10;',$value);
                    } else {
                        $roominfo[$key] = $value;
                    }
                }
        ...
        $room = new WIMBA_LCRoom();
        $room->WIMBA_setByRecord($roominfo, $this->prefix);
     ...
     *   and the value as the value of the attribute
     */
    function WIMBA_lcapi_get_room_preview($roomid)
    {
        $roominfo=array();
        $params = "&filter00=".LCAPI_ATTR_CLASSID.
                  "&filter00value=".$this->prefix.$roomid.
     ...
     */
    function WIMBA_lcapi_get_status()
    {
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_STATUS, "");
     ...
     */
    function WIMBA_lcapi_get_system_config()
    {
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_LIST_SYSCONF, '');
        preg_match("(\d*)", $response, $matches);
        $respcode = $matches[0];
        if ($respcode != 100) 
        {
    ...
    function WIMBA_lcapi_get_simulcast()
    {
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_LIST_SIMULCAST, '');
        preg_match("(\d*)", $response, $matches);
        $respcode = $matches[0];
        if ($respcode != 100) 
        {
 ...
 /**
     ...
     * @return bool - true if the user is successfuly created or already exist, false otherwise
     */
    function WIMBA_lcapi_clone_class($target, $new_id, $no_content="", $newName="",$no_roles="1")
    {
        ...
        $enc_params = str_replace(" ", "+", $params);
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_CLONE_CLASS, $enc_params);
        preg_match("(\d*)", $response, $matches);
        $lc_respcode = $matches[0];
        if ($lc_respcode == 301) 
        {
    ...
    function WIMBA_lcapi_getMP3Status($archiveId, $startGenerate,$userid)
    {
        if(empty($archiveId))
        {
        ...
        $enc_params = str_replace(" ", "+", $params);
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_GET_MP3_STATUS, $enc_params);
        preg_match("(\d*)", $response, $matches);
        $lc_respcode = $matches[0];
        if ($lc_respcode != 100) 
        {
        ...
        if($userid != null){
          $authToken = $this->WIMBA_lcapi_get_session($userid,"");
        }
        $audioFile = new WIMBA_LCAudioFileStatus($audioInfo,$authToken);
    ...
    function WIMBA_lcapi_getMP4Status($archiveId, $startGenerate, $userid)
    {
        if(empty($archiveId))
        {
          wimba_add_log(WIMBA_ERROR,WC,__FUNCTION__ ."archive should not be empty");
        ...
        $enc_params = str_replace(" ", "+", $params);
        $response = $this->WIMBA_lcapi_send_query(LCAPI_FUNCTION_GET_MP4_STATUS, $enc_params);
        preg_match("(\d*)", $response, $matches);
