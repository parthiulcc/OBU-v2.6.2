...
class WIMBA_PrefixUtil {
    
    function WIMBA_PrefixUtil()
    {
     ...
     */
    function WIMBA_getPrefix($account)
    {
        if ($account == "") 
        {
            return "";
        ...
        {
            if (!$this->WIMBA_isPrefixValid($account)) 
            {
                return null;
            } 
     ...
     */
    function WIMBA_isPrefixValid($prefix)
    {
        return ($prefix != null && preg_match(ACCEPTABLE_PREFIX_REGEX, $prefix) > 0);
    } 
     ...
     * @throws PrefixException
     */
    function WIMBA_trimPrefix($value, $prefix)
    {
        if ($prefix == "") 
        {
            return $value;
