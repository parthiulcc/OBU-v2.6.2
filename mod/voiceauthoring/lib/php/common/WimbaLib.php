...
function WIMBA_getKeysOfGeneralParameters()
{
    return array(
...
function WIMBA_getKeyWimbaClassroomForm()
{
    return array (
...
function WIMBA_getKeyWimbaVoiceForm()
{
    return array (
...
function WIMBA_getListOfWeeks($course, $current)
{
    $i = 1;
...
function WIMBA_getListOfTopics($course, $current)
{
    global $DB;
    $section = 0;
...
function WIMBA_redirection($url)
{
    header('Location:' . $url);
    exit ();
} 
function WIMBA_parentRedirection($url)
{
    echo '<script> window.top.location="' . $url . '"; </script>';
    exit ();
} 

function WIMBA_manage_error($errno, $error, $file, $line, $context)
{
    global $error_wimba;
...
function WIMBA_getRoleForWimbaTools($courseId, $userId)
{
    global $CFG;
    global $USER;
    $role = "";
...
/*
    ...
    */
function WIMBA_get_url_params($courseid)
{
    global $USER;
    global $CFG;

    $role = WIMBA_getRoleForWimbaTools($courseid, $USER->id);
 ...
 */
function WIMBA_getStudentsEnrolled($courseid)
{
  $context = get_context_instance(CONTEXT_COURSE, $courseid);
  $adminUsers = get_users_by_capability($context,'mod/voiceauthoring:presenter');
...
/* list the element of the pictures directory
    ...
    */
function WIMBA_list_dir($name, &$s)
{
    if ($dir = opendir($name)) 
    {
...
$level_values = array(
        ...
        $product == "voiceauthoring" )
    {
        ...
        $log_product = "voicetools_loglevel"; // we use the voice tools settings for all the features
    }
 ...
 * @return an array
 */
function WIMBA_convertResultFromGetAverageLengthPerUser($array){
    $result=array();      
 ...
 * @return an array
 */
function WIMBA_convertResultFromGetNbMessagePerUser($array){
    $result=array();      
    for ($i = 0; $i < count($array); $i++) 
    {
 ...
 */
function WIMBA_indexOf($string,$search)
{
    return (!(strpos($string,$search) === false))?(strpos($string,$search)):-1;
}

    
function WIMBA_endsWith( $str, $sub ) {
 ...
 */
function WIMBA_isGradebookAvailable(){
    global $CFG;
