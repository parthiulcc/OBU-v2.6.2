<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
        ...
        <div class="{type}">
            ...
            <div class="headerBarRight">
                <xsl:if test="headerBarInformations/isInstructor='true'">
                    <select id="view" onchange="WIMBA_switchView()">
        ...
        <div class="wimba_box" id="advancedPopup"
            ...
            <div class="wimba_boxTop">
                <div class="wimbaBoxTopDiv">
                    ...
                    <span title="close" class="wimba_close_box"
                        onclick="javascript:WIMBA_cancelAdvanced()">Close</span>
                    ...
                    <p style="height:20px;padding-top:10px;padding-left:20px">
                        <a class="regular_btn" href="#" onclick="javascript:WIMBA_cancelAdvanced()">
        ...
        <div id="{type}">
            <xsl:for-each select="filters/filter">
                <a href="#" class="filter{availibility}" id="filter_{name}" onclick="{action}"
                    onmouseover="javascript:WIMBA_onFilter('filter_{name}')" onmouseout="javascript:WIMBA_outFilter('filter_{name}')">
                    <span>
                    ...
                    <table id="table_list"  cellspacing="0" cellpadding="1" border="0">
                        ...
                        <xsl:for-each select="listElements/Element">
                            ...
                            <tr class="element:available {type} preview:{preview} grade:{grade}" id="{id}" name="{nameDisplay}">
                                        ...
                                        <td class="element_expand" width="20px">
                                            <xsl:choose>
                                                <xsl:when test="archives!=''">
                                                  <span class="roomWithArchives" onclick="javascript : WIMBA_hideArchive('{id}')" id="span{id}">
                                                        Display the archives
                                                    </span>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                  <span class="roomWithoutArchives" id="span{id}"
                                                  ondblclick="javascript:WIMBA_LaunchElement('{url}')"
                                                  onclick="javascript:WIMBA_clickElement('{id}','{product}','{type}')">
                                                    No Archives
                                                  </span>
                                                </xsl:otherwise>
                                            </xsl:choose>
                                ...
                                <td class="element_name" width="280px"  ondblclick="javascript:WIMBA_LaunchElement('{url}')"
                                        onclick="javascript:WIMBA_clickElement('{id}','{product}','{type}','{grade}', '{linkedActivities}')">
                                ...
                                <xsl:choose>
                                    ...
                                    <xsl:when test="type='orphanedarchivestudent'">
                                        <td class="element_{preview}_ie"  width="75px" align="center"  ondblclick="javascript:WIMBA_LaunchElement('{url}')"
                                                onclick="javascript:WIMBA_clickElement('{id}','{product}','{type}')">
                            ...
                            <xsl:for-each select="archives/archive">
                                <tr class="archive element:available parent:{parent} hideElement preview:{preview}" id="{id}" name="{nameDisplay}" >
                                    <td class="element_expand"  width="20px" onclick="javascript:WIMBA_clickElement('{id}','{product}','{type}')" ondblclick="javascript:WIMBA_LaunchElement('{url}')">
                                        <span class="subItem" id="{id}subItem"> </span>
                                    </td>
                                    <td class="list_name" width="280px" onclick="javascript:WIMBA_clickElement('{id}','{product}','{type}','{grade}','{linkedActivities}')" ondblclick="javascript:WIMBA_LaunchElement('{url}')">
                                        <span class="">
                                            <xsl:value-of select="nameDisplay"/>
                                        </span>
                                    </td>
                                    
                                    <td class="element_{preview}_ie" align="center"  id="{id}Availability"  width="75px" onclick="javascript:WIMBA_clickElement('{id}','{product}','{type}')" ondblclick="javascript:WIMBA_LaunchElement('{url}')">
                                        <span alt="{tooltipAvailability}" title="{tooltipAvailability}">
                                            This element is <xsl:value-of select="preview"/>
                                        </span>
                                    </td>
									...
									<td  width="100px" onclick="javascript:WIMBA_clickElement('{id}','{product}','{type}')">
	                                	<span><xsl:text>&#160;</xsl:text></span>
	                                </td>
                                </tr>
                            </xsl:for-each>
            ...
            <ul class="module_tab">
                <xsl:for-each select="tabsInformations/tabInformation">
                    <li class="{style}"  id="tab{id}">
                        <span id="tab{id}span"
                            onclick="javascript:WIMBA_onTab('{id}','{additionalFunction}')">
                            <xsl:choose>
    ...
    <xsl:template match="windowsElement[type/text()='message']">
        <div class="{type}" id="{type}">
            ...
            <span class="close" onclick="WIMBA_closeMessageBar()">Close</span>
        </div>
    </xsl:template>
                    ...
                    <div id="validationBar" class="validationBar">
                        <table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-top:solid 1px #808080;padding-left:15px">
                            <tr>
                                ...
                                <td align="left" onclick="WIMBA_launchPodcast('{podcasterComponent/url}')" class="link">
