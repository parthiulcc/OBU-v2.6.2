...
function verifyFormLiveClassRoomUpdate(url) {
	...
	if (!longnamePattern.test(theForm.longname.value)) {
		errorMessage += "Please fill in a Title that is 1-50 alphanumeric or space characters or - / : ' ? ! ( ) @ and begins with a letter or _\n";
	}
	if (spacesPattern.test(theForm.longname.value)) {
		errorMessage += "The Title must contain alphanumeric characters or - / : ' ? ! ( )\n";
	}
...
function verifyFormVoiceBoard(url) {
	...
	d.action = url;
	var spacesPattern=/^\s+$/g;
	if (spacesPattern.test(d.longname.value)) {
   ...
   // alert(document.getElementById("ddd").options[1].value);
    ...
    if (gradeSetting != null && gradeSetting.checked) {
      ...
      // Check that Points possible is positive
      if (gradePoints.value < 0) {
        alert("A non-negative numeric value must be entered: Points Possible.");
        gradePoints.value = ""; //empty the field
        gradePoints.focus();    //give the focus
        return false;
      }
