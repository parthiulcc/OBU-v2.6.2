...
function voiceemail_add_instance($voicetool) {
  ...
  /// of the new instance.
    ...
    $vtAction=new WIMBA_vtAction($USER->email,$paramVMail);
    
    $vt=$vtAction->WIMBA_createVMmail($voicetool->name);
    if($vt === false || $vt->error == "error"){
        print_error(get_string('problem_vt','voiceemail'));
        return false;
    }
    $resource_id= WIMBA_storeVmailResource($vt->WIMBA_getRid(),$voicetool->course);//default availability
...
function voiceemail_update_instance($voicetool) {
  ...
  /// will update an existing instance with new data.
    ...
    $vt = $vtAction->WIMBA_updateVMmail($voicetool->r_id,$voicetool->name);
    if($vt->error == "error"){
        return false;
    }
    
    $resource_id= WIMBA_updateVmailResource($vt->WIMBA_getRid(),$voicetool->course);//default availability
    $resource_id = $DB->get_field('voiceemail_resources', 'id', array('rid' => $vt->WIMBA_getRid()));
    if(empty($resource_id))
    {
        return false;  
    }
...
function voiceemail_WIMBA_get_url_params($courseid)
{
    global $USER;
    global $CFG;
...
function voiceemail_reset_userdata($data) {
   ...
   if (property_exists($data,'reset_content_voiceemail') && $data->reset_content_voiceemail) {
