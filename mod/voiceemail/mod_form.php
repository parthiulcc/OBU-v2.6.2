...
$PAGE->requires->js('/mod/voiceemail/lib/web/js/lib/prototype/prototype.js');
$PAGE->requires->js('/mod/voiceemail/lib/web/js/wimba_ajax.js');
$PAGE->requires->js('/mod/voiceemail/js/mod.js');
$PAGE->requires->js('/mod/voiceemail/js/init_calendar_checkbox.js');
    ...
    function definition() {
        ...
        if(isset($this->current->update))
        {
            ...
            if($resource->error != "error"){
                $resourceOptions = $resource->WIMBA_getOptions() ;
                $maxLength = $resourceOptions->WIMBA_getMaxLength();
                $resourceAudio = $resourceOptions->WIMBA_getAudioFormat();
                $audioQuality = $resourceAudio->WIMBA_getName();
                $replyLink = $resourceOptions->WIMBA_getReplyLink();
                $prefilledSubject = $resourceOptions->WIMBA_getSubject();
            }
            else
            {
                $error=true;
        ...
        $url_params = voiceemail_WIMBA_get_url_params($COURSE->id);
        ...
        //recipients
        $radioarray=array();
        $radioarray[] = $mform->createElement('radio', 'recipients_email', '', get_string('instructors', 'voiceemail'), 'instructors');
        $radioarray[] = $mform->createElement('radio', 'recipients_email', '', get_string('students', 'voiceemail'), 'students');
        $radioarray[] = $mform->createElement('radio', 'recipients_email', '', get_string('all', 'voiceemail'), 'all');
        ...
        $prefilledarr = array();
        $prefilledarr[] = $mform->createElement('radio', 'pre_filled_subject', '', get_string('no', 'voiceemail'), 'false', array('onclick' => 'enableSubject()'));
        $prefilledarr[] = $mform->createElement('radio', 'pre_filled_subject', '', get_string('yes', 'voiceemail'), 'true', array('onclick' => 'enableSubject()'));
        $subAttribs = null;
        if ($prefilledSubject == '') {
            $subAttribs = array('disabled' => true);
        }

        $prefilledarr[] = $mform->createElement('text', 'subject', $prefilledSubject, $subAttribs);
        ...
        //include reply links
        $replyarr = array();
        $replyarr[] = $mform->createElement('radio', 'reply_link', '', get_string('no', 'voiceemail'), 'false');
        $replyarr[] = $mform->createElement('radio', 'reply_link', '', get_string('yes', 'voiceemail'), 'true');
